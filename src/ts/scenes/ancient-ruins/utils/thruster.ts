...
export class Thruster {
    ...
    private _createFlames(position: [number, number, number]): void {
        ...
        const v1 = new Vector3(0.75, 0, 0.36);
        const v2 = new Vector3(0.05, 0, 0);
        const v3 = new Vector3(0.75, 0, -0.36);
        ...
        const meshBlue = new Mesh(geometry, this._orangeFlameMaterial);
        meshBlue.position.set(position[0], position[1], position[2]);
        meshBlue.name = `flame-${index}`;
        this._flames.push(meshBlue);
        this._scene.add(meshBlue);
        meshBlue.visible = false;

        const meshPink = new Mesh(geometry, this._redFlameMaterial);
    ...
    public rotate(rotation: Euler) {
        this._flames.forEach(flame => {
            flame.rotation.set(flame.rotation.x, flame.rotation.y, rotation.z);
            flame.updateMatrix();
        });
    }
}