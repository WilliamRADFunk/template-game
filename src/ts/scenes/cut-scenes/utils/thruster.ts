...
export class Thruster {
    ...
    private _createFlames(position: [number, number, number], scale: number): void {
        ...
        const v1 = new Vector3(0.5 * scale, 0, 0.12);
        const v2 = new Vector3(0.05, 0, 0);
        const v3 = new Vector3(0.5 * scale, 0, -0.12);
        ...
        const meshBlue = new Mesh(geometry, this._blueFlameMaterial);
        meshBlue.position.set(position[0], position[1], position[2]);
        meshBlue.name = `flame-${index}`;
        meshBlue.scale.set(scale, scale, scale);
